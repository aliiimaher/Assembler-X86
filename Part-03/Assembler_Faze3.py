### Program Name: Design An Assembler Part3.
...
### Improvements: In this part we add short JMP instruction. 
### Note that we support the last parts (MOD = 11) and (MOD = 00) as well.
...
### Warning 3: Just One Jump Is Allowed :)
...
byteCount = 0   # Shows the jump amount based on byte size.
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
def select_Instruction(asmCode, roundd):
    ...
    elif (asmCode[0].upper() == "JMP"):
        retCode = JMP_Instruct(asmCode[1].upper())
    
    elif(":" in asmCode[0]):    # This means we have a label.
        global valJmpTo
        jmpTo = (asmCode[0].replace(":","")).upper()
        valJmpTo = byteCount
        retCode = ''
...
def calculate_distance():
    ...
    NoBytes = valJmpTo - valJmpFrom
    distanceLoc = str(NoBytes)  
    if (NoBytes < 0):
        NoBytes *= -1
        distanceLoc = hex(255 - NoBytes - 1)
    
    else: # NoBytes > 0 ---> First JMP
        distanceLoc = '0x' + distanceLoc    
...
# MOD = 11: Reg to Reg
    ...
    if (MOD == "11"):
        ...
        if opd1 in reg_8 and opd2 in reg_8:
            ...
            byteCount += 2
            machineCode = '\\' + (" 0x00") + "\\ " + hex(int(MOD + reg_Value[opd2] + reg_Value[opd1], 2))
            ...
            byteCount += 2
            machineCode = '\\' + (" 0x01") + "\\ " + hex(int(MOD + reg_Value[opd2] + reg_Value[opd1], 2))
...
# MOD = 11: Reg to Reg
    ...
    if (MOD == "11"):
        ...
        if opd1 in reg_8 and opd2 in reg_8:
            ...
            opd2 = reg_8.index(opd2)
            byteCount += 2
            machineCode = '\\' + (" 0x28") + "\\ " + hex(int(MOD + reg_Value[opd2] + reg_Value[opd1], 2))
            ...
            opd2 = reg_32.index(opd2)
            byteCount += 2
            machineCode = '\\' + (" 0x29") + "\\ " + hex(int(MOD + reg_Value[opd2] + reg_Value[opd1], 2))
...
# MOD = 11: Reg to Reg
    ...
    if (MOD == "11"):
        ...
        if opd1 in reg_8 and opd2 in reg_8:
            ...
            opd2 = reg_8.index(opd2)
            byteCount += 2
            machineCode = '\\' + (" 0x20") + "\\ " + hex(int(MOD + reg_Value[opd2] + reg_Value[opd1], 2))
            ...
            opd2 = reg_32.index(opd2)
            byteCount += 2
            machineCode = '\\' + (" 0x21") + "\\ " + hex(int(MOD + reg_Value[opd2] + reg_Value[opd1], 2))
...
def JMP_Instruct(label):
    
    global valJmpFrom
    jmpFrom = label.upper()
    valJmpFrom = byteCount
    machineCode = '\\ xEB' + '\\' + ' TTEMPP'
...
for roundd in range(0, len(asmCode)):
    ...
    if (machineCode):
        finalRes += machineCode

# # Assign Jump Value
distance = calculate_distance()
if ('TTEMPP' in finalRes):
    finalRes = finalRes.replace("TTEMPP", distance)
